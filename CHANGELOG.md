# 2026-09-09 - Supplied star art, supplied award sounds, heavier landing - 1.0.19 (vc21)

- **The stars are your art now.** One supplied sheet sliced by `scripts/dev/prepare_star_kit_art.gd` into three runtime derivatives - the gold star, the gold outline placeholder, and the haloed star used as a bloom. The rects were measured by walking the sheet's alpha columns rather than eyeballed, and the sheet size is asserted before they are used. The result popup, the level map and the level-screen header draw the same three textures, so the stars are the same object everywhere they appear.
- **No hard cut and no erosion on these derivatives, unlike the power sheets.** Those two passes exist to strip a red fringe this source does not have, and the glow variant is deliberately semi-transparent across most of its area: a 0.5 alpha cut would delete the halo outright and an erosion pass would chew the anti-aliased edge off every star. Only sub-threshold noise is cleared, at the same 0.01 the gem derivatives use, then each variant is trimmed to its own alpha bounds and scaled to 256 on the longest edge with aspect preserved. A star is wider than it is tall, so every consumer sizes by height and derives the width - including the row's slot pitch, which would otherwise overlap three stars.
- **The landing is much heavier, and the rest of the screen steps back for it.** A star grows from 0.10 to a 1.62 overshoot, bounces once, and settles on exactly its placeholder; the supplied glow star blooms behind it at 1.85x and fades across the settle; a ring and ten rays fire at the peak of the growth. While the sequence runs, the subtitle, the coin figures, the transition line and all three buttons fade to 34% and then restore - held back rather than hidden, because hiding them would reflow the column and move the star row mid-animation.
- **The award sounds are the supplied recordings.** The star ding and the treasure shine replace the generated chimes, preserved under `assets/sound/` and copied byte-identical into `assets/runtime/audio/` (both are already short, already start on their attack, and Godot decodes MP3 natively - `merge-basic.mp3` already ships this way). `run_sound_privacy_link_tests` asserts both by stream identity, so a later refactor cannot quietly put procedural chimes back.
- **The star pitch ladder is an explicit table.** `[1.0, 1.12, 1.25]` rather than a base plus a step: three values are the tuning and a formula only hid them. A fourth star, should the count ever change, continues by the last interval and stays inside the audio service's 0.80-1.40 clamp. Asserted directly.
- `star_complete` stays a generated chime, pitched under the ding, so the flourish that closes the sequence is plainly not a fourth star.

# 2026-09-09 - Star row sequencing, award rework, header cleanup - 1.0.19 (vc21)

- **Stars fill left to right now, which reverses a decision made earlier the same day.** The row lit the exact objectives that were met, so earning the first and third left a visible gap. That is the more precise answer and the wrong one: a player who earned two stars expects to see two of them together, and a gap disagreed with the map and the header, which are both counts. The row is a score out of three; the caption underneath still names each earned objective as its star lands, so nothing about *which* objectives were met is lost.
- **The award grows out of its own placeholder rather than flying in.** The previous version dropped each star in from 2.9x with a spin, which the eye has to track before it can read the star - three in a row read as busy rather than as earned. Each star now grows from the empty placeholder it will occupy, overshoots to 1.34, and settles back onto exactly that size and position. The ring and rays fire at the peak of the growth instead of during it, so the impact reads as the star landing on its slot.
- **Removed the filled disc behind each lit star.** It read as a badge the star was sitting on rather than as light coming off it.
- **The level-screen header is just the star total.** The star is 26px -> 20px, and the "next chest in N levels" countdown is gone: it was on screen permanently, changed by one per level, and named something the player can do nothing about - the chest is drawn on the path where they will actually reach it. A chest that is earned and still unopened is still announced, because that one is actionable, and the separator dot is hidden when nothing follows it.

# 2026-09-09 - Star award polish, treasure pacing, Level Ready hierarchy - 1.0.19 (vc21)

- **Fixed the star row claiming stars the player did not earn.** `award()` raised a lit *count*, so lighting the third star of a `[earned, missed, earned]` result lit all three - the row showed three gold stars over a caption reading "2 of 3". A count cannot express a gap, and a gap is the ordinary case. Lit state is per star now and the tally is derived from it, so the two cannot drift apart.
- **The row also stopped pre-lighting stars the player already held.** Storage is a count, not a set, so "two already" cannot say which two; pre-lighting the first two claimed a star that may have been missed. The row starts empty and lights exactly what the attempt earned.
- **The award actually feels like one now.** Each star drops in from 2.9x with a spin, lands on an overshoot under a white bloom, and leaves an expanding ring and a ten-ray burst; a settled row keeps a slow shimmer so a finished result is not a static picture. Stars are 62px -> 78px. Full marks says `PERFECT!` with a kick instead of tallying `3 of 3 stars`.
- **Stars, treasure openings and treasure claims all have their own sound now, and the series rise.** Three new chime identities in `GameConfig.AUDIO_TONES`, played through the existing bounded pitch parameter at `star_award_pitch(index)` - three stars are one climbing phrase rather than the same note three times, and the last star resolves on a lower `star_complete`. Claims climb the same way. The audio cache bound moved 25 -> 28, and the existing assertion on that bound is what caught the additions.
- **The treasure fanfare was playing a second too early.** It fired on `present()`, over a chest that was still shut and still waiting for the tap that opens it. It now fires on `chest_opened`, at the burst. `chest_opened` and `reward_claimed` are presentation signals; the controller owns every cue, so no UI layer reaches the audio service directly.
- **Bonus treasures no longer arrive back to back.** The 15% roll is correct on average over hundreds of levels, but a hash is only uniform in the large - it fired on levels 1, 2 and 5, and five times in the first twenty, which is exactly the stretch a new player sees and reads as "a treasure every level". An accepted drop now also needs the previous two levels to have failed the roll, so two drops are always at least three levels apart and the effective rate settles at 10.2%. It remains a pure function of the level number, so replays are unchanged.
- **Level Ready gives the mascot back its screen.** The objectives sat above it inside a bordered card, so they were the first thing the eye landed on and the mascot had been pushed down the panel. They are below it now, as a plain list with no frame: a second panel inside a popup was competing with the popup it lived in.
- **Fixed statically lit stars drawing at 2.9x.** `_draw` treats `_pop` as arrival progress and `resize()` zero-fills; zero reads as mid-arrival, so every star that had never been animated - the level-screen total, the map rows, the level-start list - drew at the full arrival scale. Caught on the device, where the header star towered over the line of text beside it. 1.0 is settled and is now the default. The measured box also stopped reserving room for the burst ring, which had made every static row twice the height of its star.
- **HOME is no longer disabled during the star sequence.** A proof capture caught it looking exactly as it does when live while refusing taps - its plate has no disabled art - and a dead exit is worse than an early one. COLLECT and DOUBLE COINS stay gated.

# 2026-09-09 - Level stars, and levels that stay the level you played - 1.0.19 (vc21)

- **Every level now asks for three stars.** Complete it; finish inside a shot budget; and one of three rotating objectives - merge a stated number of gems, reach a stated combo depth, or win without spending a power. The rotation is a hash of the level number rather than `level % 3`, so it is not a metronome, and across levels 1-120 it comes out at 34 merge / 43 combo / 43 no-power.
- **No star threshold is authored by hand.** `LevelSolver.simulate()` now reports the merges it performs as well as the shots, and both numeric stars are sized against a play-out of that exact level. This is the direct lesson of the shot ladder that preceded the solver: it was written by hand, it was never printed, and it made every limited level unwinnable for months. `scripts/dev/print_level_star_audit.gd` prints the whole table so the numbers can be read rather than assumed.
- **Measured headroom: the tightest shot star in 120 levels leaves 25% over a perfect play-out**, and on a limited level the star always sits strictly inside that level's own shot budget - otherwise it would be handed out free with the win. Both bounds are asserted for all 120 levels rather than spot-checked.
- **The merge star sits deliberately *below* a perfect play-out.** A real player takes 30-70% more shots, which is more material and more merges, but also strands gems the play-out pairs perfectly, and the net of those two is not something the config can know. A slightly easy star is a mild waste; a star nobody can reach is a promise the level-start screen made and broke. So the bar is 90% of the demonstrated count, rounded down to a multiple of five - "Merge 95 gems" rather than "Merge 98 gems", which reads as a number that fell out of a formula.
- **Level Ready shows the three objectives with empty stars**, and lights the ones already held so a replay shows which is still missing. The wording comes from `LevelStars` through the controller snapshot, so the screen cannot promise a different objective from the one the result popup judges.
- **Level Complete awards the earned stars one at a time**, naming each objective under the row as it lands, and COLLECT and DOUBLE COINS stay locked until the last one has. Without the gate a fast tap on the popup skipped an award the player never saw. HOME is deliberately left live - a proof capture caught it looking exactly as it does when enabled while refusing taps, since its plate has no disabled art, and a dead exit is worse than an early one. A failed result shows no star row and gates nothing.
- **Stars are evaluated, banked and reported before the popup opens** - the same rule the treasure follows. Per-level storage is monotonic, so replaying an old level to chase a missed star can never cost the ones already held.
- **The level screen totals the stars beside the chest line**, and every cleared node on the map carries its own three-star row. Both are drawn, not built from Controls, for the same reason every other ornament on that map is: it spans a thousand levels.
- **A level you have played is now pinned to a snapshot of itself.** Generation was already a pure function of the level number and still is, so replays were already identical - but purity is with respect to *our* generator, and retuning a template or bumping `GENERATOR_VERSION` silently rewrites every level already played. `LevelRecordService` records seed, gem identities, opening board, launcher queue, target cards, shot limit, background and table the first time a level is entered, and restores them on every later entry. A level already on file is never re-pinned; an unconditional write would re-pin it to whatever the current generator produces on each visit, which is the opposite of the point.
- **Fixed the drift detector reporting a mismatch on every level.** `LevelRecordService.matches()` compared `str()` of the stored and regenerated values, and a ConfigFile round trip does not preserve Dictionary key order - two byte-identical opening boards came back with their keys in a different order and compared unequal. It compares structurally now, with ints and floats reconciled.
- Records live in their own `user://level_records.cfg` rather than in the progression save, which is read on every launch and written on every coin transaction.
- Simulation, merge, launcher, collision, table geometry and level generation itself are untouched.

# 2026-09-09 - The treasure ceremony, and a treasure the win can drop

- **Treasures are a fullscreen ceremony now, not a panel row.** `TreasureOverlayLayer` (layer 70) owns one presentation used by all three sources: the daily chest, the twenty-level milestone chest, and the new post-win drop. The chest arrives large and centred over a drawn light effect - a slow fan of god rays, two breathing halos and eighteen sparkles on independent orbits - and waits under `TAP TO OPEN`. Tapping it rattles the lid, swells it, swaps to the open art at the peak of the swell under a burst of that same light, and holds so the burst is seen before anything covers it. All of it is immediate-mode drawing in `TreasureVfx`; a node-per-ray design would have allocated a few hundred nodes on a low-end phone for an effect lasting a few seconds.
- **Rewards are claimed one at a time, by tapping.** Each reward appears alone under `TAP TO CLAIM`, swells and rises away when tapped, and the next follows after a short beat. Coins are one claim; each granted power is one claim carrying its own count, so three Switches are one card rather than three taps. Nothing advances on a timer, which is the whole point - a reveal that plays itself past the player is a cutscene, not a reward.
- **A level win can now drop a treasure, and it lands before the win popup.** Clearing a twentieth level hands over that milestone chest on the spot instead of leaving it waiting on the map to be noticed; every other win rolls a 15% bonus of 120 coins and one power. `TreasureDrop` decides both, and it is a pure function of the level number rather than of the clock or a live RNG - a treasure that appeared only on a first clear would make replays quietly worse than the original.
- **A milestone is still earned exactly once.** A chest already in `claimed_chests` never drops again, and a chest granted by a win is recorded there, so the level map shows it opened rather than offering it a second time. Chests earned before this change are still tappable on the map and open the same ceremony.
- **Fixed the treasure being able to masquerade as the level's own reward.** A milestone pays 800 coins into the same balance Level Complete reads. The level's reward is now captured before any treasure is granted, and the treasure raises `level_start_coins` alongside `coins`, so the popup reports what the level paid and Double Coins offers to match only that.
- **The daily chest opens by tapping the chest.** Reaching past a picture of a chest to press a button labelled CLAIM was the least direct way to open one. The art carries an invisible button; the plate beside it stays because it is what says LOCKED and DONE, and its caption is now OPEN. Every state on the tap target is an explicit `StyleBoxEmpty` - `flat = true` suppresses only the draw call, so the button would otherwise have inherited the shared gem plate and drawn it behind the chest.
- **The win popup's mascot is unchanged and now has a regression proving it.** The treasure only delays the popup, but the mascot frames are a static cache shared by every instance and the ceremony runs its own always-processing layer, so the suite drives a whole ceremony and then asserts the popup still queues its reaction and travels the happy track.
- **`run_reward_feedback_v3_tests` was updated because the behaviour genuinely changed, not to make it pass.** It drives a win to Level Complete and measures the level's own coins exactly; it had been running on whichever level the save held, so a post-win treasure both added coins and held the popup it waits for. Those cases now pin an odd, treasure-free level, chosen through `TreasureDrop` rather than hardcoded, so the pin cannot drift if the drop rules change.
- Simulation, merge, launcher, collision, table geometry and level generation are untouched.

# 2026-09-07 - A normal scroll, the v7 launcher icon, and a real asset purge - 1.0.18 (vc20)

- **The level map scrolls with an ordinary `ScrollContainer` now.** It had been given hand-rolled flick physics - a scroll offset, a velocity, an exponential friction curve - because an earlier attempt at a container appeared not to scroll at all. The container was never at fault. The map was `MOUSE_FILTER_STOP`, so the drag never reached it; the workaround bolted on for that wrote `scroll_vertical` straight from each drag event in whole pixels with no momentum, which is what "not smooth" actually was, and the rewrite that followed replaced the workaround instead of the cause. `LevelMapView` is `MOUSE_FILTER_PASS` and never calls `accept_event()` on a drag, so every gesture reaches the container and the map gets exactly the scrolling every other Android app has, momentum and rubber-band included. The map is now the container's content: it declares its full height, draws in content space with no transform of its own, and is told which slice is on screen through `set_window()`. Drawing is still windowed, so the cost of opening the map at level 4000 is what it was at level 4.
- **Fixed the level-select suite printing PASS while three of its cases were dead.** A GDScript runtime error aborts the case it happens in but neither stops the runner nor fails the process, so calls to `set_window()` and `overlay.scroll` - which the previous scroll rewrite had deleted - failed silently on every run and the suite reported green. Each case now signs a register on its way out and the run fails if a signature is missing. Two further suites, `run_game_flow_reward_splash_tests` and `run_no_ads_available_v1_tests`, are green in the same false way for the same reason and are **not** fixed here; they are recorded in `CURRENT_STATE.md`.
- **Fixed two suites that passed or failed depending on the calendar date.** `run_retention_daily_missions_v2_tests` and `run_mission_notification_v1_tests` hard-coded `"merge"` as the event they recorded against the first daily mission. The daily set is rolled from the date and the easy pool holds a `target_complete` objective as well as two merge ones, so on a day that rolled `target_complete` nothing advanced and nine assertions failed. Pinning the date is not available - `record()` re-derives today internally and would roll a pinned state forward - so both suites now drive whatever type actually rolled.
- **The supplied v7 illustrated logo replaces the previous one in the two roles that variety held**: the legacy 192px launcher icon and the adaptive icon's background layer. The transparent mark is untouched, so Home, the Android launch screen and the engine boot splash still use v6 - baking an illustrated background into any of them would show as a square over artwork they already composite against.
- **The adaptive icon's surround is an edge extension now, not a blur of the whole illustration.** Two other surrounds were tried on the new art and both were visibly wrong: a coarse blur averaged the gold frame and brown wood into the ring and produced a muddy band that read as a border, and a fine blur left a ghost of the wordmark out in the ring so the icon showed its logo twice. Each surround pixel now takes the colour of the nearest pixel on the artwork's edge, which on this art is its lilac bokeh, so the join has nothing to reveal it. Verified under circle, squircle and rounded-square masks before shipping.
- **Deleted the superseded brand assets that three refreshes had left behind**: fifteen runtime derivatives, nine orphaned `.import` sidecars whose images had been removed in an earlier pass without them, and `prepare_majestic_gems_launcher_v2.gd`, whose only outputs were those derivatives. They had survived because `run_branding_push_line_tests` was still loading the v4/v5 files and asserting the dimensions of art the game had not shipped in two brand refreshes; that suite now asserts against the live v6/v7 art. Supplied sources under `assets/logo/` are all preserved, per the standing rule that originals are kept and only derivatives are disposable.
- **Google Play Games Services: nothing to remove.** It was built and fully removed on 2026-08-28 at the user's request. Verified absent from source and from the shipped `1.0.18` APK's dex, which contains no `com/google/android/gms/games` or `PlayGamesSdk` reference in any of its three DEX files. AdMob and Firebase Analytics are unaffected and remain in the build, as intended.
- **Daily missions roll at local midnight, not 24 hours after they are seen.** `DailyMissionService` keys the set on `Time.get_date_string_from_system()`, so a player who first opens the game at 23:50 gets a new set ten minutes later. This is existing behaviour, is unchanged here, and is recorded rather than fixed.
- Gameplay is untouched: no simulation, `GameConfig`, launcher, aim, merge, collision or scoring file is modified in this pass.

# 2026-09-05 - Release identity correction and final optimization - 1.0.18 (vc20)

- Restored the canonical production identity to versionName `1.0.18` / versionCode `20`. Codes 21-27 were consumed only by local/test APK iterations and are not the product release line.
- Removed five unreachable legacy runtime derivatives (v3 fallback logo, v3/v4 app icons, and v4/v5 system splashes). Supplied originals remain preserved; current v6 branding remains authoritative.
- Retained the owned level-map scrolling, seam-proof splash, and cross-suite save-isolation fixes described below as the final `1.0.18 (20)` codebase.

# 2026-09-05 - Real scroll rework, splash that cannot band - test iteration

- **Rewrote level-map scrolling instead of tuning it again.** The map no longer lives in a `ScrollContainer`. Two things were wrong with that: the map had to *be* the content, which for a thousand levels is a node 186,000px tall being re-laid-out on every scroll; and because a drag starting on a custom-drawn child was not reliably claimed by the container, a handler had been added that wrote `scroll_vertical` straight from each drag event in whole pixels with no velocity - so the map stepped while the finger moved and stopped dead the instant it lifted. That, not frame rate, is what "not smooth" was. The view now owns a scroll offset, stays exactly viewport-sized, shifts its drawing with one `draw_set_transform`, and runs its own flick physics with exponential friction so a release glides and settles. The regression asserts the map follows the finger to the sub-pixel, carries momentum on release, comes to rest, and stops when touched mid-glide.
- **The splash cannot band any more, and it is proved before the file is written.** Godot fits the splash inside the window and fills the rest with `boot_splash/bg_color`, so any border pixel that differs shows as a bar. The previous gradient reached `bg_color` at the corners but was still at `#3D1258` along the top edge centre, which is the shortest distance from the centre - so it banded exactly where it was looked at. The falloff is elliptical and completes at the nearest edge now, and `_assert_border_is_bg` refuses to write a splash whose border differs by more than one 8-bit step.
- **Fixed the intermittent suite failure at its real cause.** It was not frame deltas. Every suite runs in its own process but they all share `user://`, so daily-mission progress banked by an earlier run was still there - and a merge that completes a daily mission credits coins, which is exactly what the failing assertion measured. The case clears saved progress first. Verified over three consecutive clean full passes.
- This work was temporarily assigned test-only version values during local APK iteration; those values are retired and were never the canonical product version.

# 2026-09-05 - Gradient splash that fits, real scroll fix, chest line - 1.0.21 (vc23)

- **The splash is a gradient again, and this time it fits every screen.** The gradient was never the problem - a 720x1280 raster was. Godot letterboxes a fixed-size splash with `boot_splash/bg_color`, and that image faded to a colour the fill did not match, so the join showed as a band on a tall phone. Flattening it to a solid colour removed the band by removing the gradient, which is not a fix. It is now a radial gradient that resolves to exactly `bg_color` at its own edges, so whatever is letterboxed away is replaced by the colour it had already reached and the join cannot be seen at any aspect.
- **The brief Android window background before the engine starts is still a flat colour, and that is not for want of trying.** A shape drawable was authored and wired through `gradle_build/custom_theme_attributes`, the documented hook, since `themes.xml` is regenerated on every export. Dumping the built APK showed Godot still writing `android:windowBackground=#ff1c0734` over it, so the drawable was packaged but never used. Rather than ship dead configuration that looks like a fix, it was removed. What the player actually watches - the engine splash above - is a real gradient.
- **Fixed the level map scroll properly, by measuring instead of guessing.** A repaint cost 4.13ms on desktop, which is a dropped frame on a phone every time the visible range changed. Three causes, all measured: `get_string_size` was being called for every visible number on every repaint and alone cost 0.94ms - it is cached now, since the answer only depends on the digits and the size; the drop shadow was a `StyleBoxFlat` with `shadow_size`, which rebuilds a shadow mesh on every draw and was drawn once per visible node, so it is the plate's own nine-patch again, dark and offset; and the level numbers were outlined with eight offset `draw_string` calls each, which is now one `draw_string_outline`. Repaint cost is now below the measurement noise floor.

- **Fixed the level map advertising a countdown while an earned chest sat unopened on the path.** At level 21 with chest 1 waiting a row below, the header read "Next chest in 20 levels" - it counted to the *next* chest and ignored the one already unlocked, which actively talks a player out of collecting a reward they have earned. An unopened earned chest now outranks the countdown and the line reads "Chest ready - tap to open". Covered across four states: chest waiting, chest taken, one level short, and a new player.
- **Fixed an intermittently failing regression suite.** `run_animation_audio_back_privacy_polish_tests` drives merge and target presentation by hand, one explicit advance at a time, but left the controller's own `_process` running - so real frame deltas advanced the same state in between and the outcome depended on how busy the machine was. It passed in isolation and failed roughly one run in five inside a full suite pass, which is worse than a hard failure: it trains everyone to re-run instead of read. The controller is frozen for that case now, and the suite was confirmed over two consecutive clean full passes.
- Bumped to versionCode 22 / versionName 1.0.20.

# 2026-09-05 - Coin farming exploit, icon zoom, splash seam, map scroll - 1.0.19 (vc21)

- **Fixed a coin exploit: an abandoned attempt's earnings became real money.** `restart()` never rolled the display balance back. Every comment in the controller claimed it did - it only reset `score` - so hitting a target, leaving the level unfinished, and re-entering it left `coins` inflated, Home displayed the inflated figure, and `_on_level_chosen` then banked it into `level_start_coins`, where it was genuinely spendable and compounded on every repeat. Only a cold start revealed the truth, because nothing was ever persisted. `restart()` now rolls `coins` back to the banked balance, entering a level no longer banks anything, and leaving one discards unresolved earnings immediately so Home and the map never show a figure the economy will not honour. `run_coin_economy_v1_tests` reproduces the exact sequence that was reported and fails without the fix.
- **Fixed the launcher icon looking zoomed in.** An adaptive icon is authored at 108dp but only its middle 72dp is ever displayed - the outer band exists for the mask and for launcher parallax - so scaling the artwork to fill the canvas handed every launcher a 1.5x enlargement of it. The illustration is now drawn at the 72dp viewport, with the ring around it filled by a blur of the same artwork: a flat fill made the icon read as a picture on a card, and an un-blurred copy showed the logo twice.
- **Fixed the launch background not fitting tall screens.** `boot_splash/image` was a 720x1280 gradient whose edges did not match `boot_splash/bg_color`, so on a 20:9 phone the letterboxing showed as a band across the top and bottom. It is now the v6 mark on a plate filled with exactly the boot background colour, which makes the letterboxing invisible at any aspect. It also still carried the previous logo.
- **Fixed the level map scrolling roughly.** Two causes. The map repainted on every scroll event even when the same slots were still on screen, and the current level's breathing halo forced a full repaint sixty times a second - a dozen nine-patched plates, their laurels, the path strokes, the scattered gems and about a hundred `draw_string` calls for the outlined numbers, every frame, to animate one ring. The halos and chest sparkles moved to their own canvas item behind the map, and the map itself now repaints only when the visible slot range actually changes. A touch deadzone stops a wobbling tap from starting a scroll.
- Bumped to versionCode 21 / versionName 1.0.19.

# 2026-09-05 - Brand refresh v6, reminders in the pause menu, and 1.0.18 (vc20)

- **The v6 logo is live, with each supplied variety doing the job it can actually do.** The transparent mark is used on Home and on the Android launch screen, because both composite it over a background they already have - the launch screen over the `#1C0734` window colour, Home over the garden art - and a baked-in background would show as a visible square on both. The illustrated square is used for the launcher icon, which is a fixed square with nothing to composite against.
- **The adaptive launcher icon is the illustration and nothing else.** It is the background layer with a deliberately empty foreground, so the icon looks the same on Android 7 and Android 14. It is full bleed rather than inset into the adaptive safe zone: inset was tried first and left a flat purple border, so the icon read as a picture pasted on a card. Full bleed is safe because of where the wordmark sits - a launcher mask crops to roughly the inscribed circle, and the wordmark's half-width is about 162 against a 216 radius, so what the mask removes is garden. `scripts/dev/preview_adaptive_icon_masks.gd` renders the composited icon under circle, squircle and rounded-square masks so that crop is checked before shipping rather than discovered on a handset.
- Rewrote `prepare_brand_refresh_v1.gd`, which previously wrote the transparent mark into a file still named `logo_v5`, generated a hand-rolled gradient splash nothing referenced, and produced an "app icon" that was the transparent logo on a transparent canvas. All five derivatives are now v6-named and generated from the two sources with one documented rule each.
- **Home's logo box changed shape with the art**, 424x259 to 360x340. The mark is drawn with its aspect preserved, so the old wide box letterboxed the new square crest down to 259 across and made it look small on the screen.
- **Added the REMINDERS switch to the in-game pause settings.** It only existed in Home settings, which is not where a player reaches for settings mid-session - so for anyone who never opened Home settings, the toggle effectively did not exist. Both panels now carry the same three switches.
- Gave reminders its own icon. It was inheriting the speaker glyph from the sound row, which said the wrong thing: the icon set has no bell, so it uses the kit's calendar badge, which at least means "daily".
- Bumped to versionCode 20 / versionName 1.0.18 and delivered a release-signed APK.

# 2026-09-05 - Mood mascot, daily reminders, and one popup shell

- **Revisions after review** (details in the report): popup expressions now play from neutral once the popup has finished arriving, rather than snapping to the final frame during the entrance; the idle breathing loop and a `TRANS_BACK` overshoot are both off in popups, which were the two bounces. The shots counter moved out of the centre of the HUD into a compact pill under Coins, freeing the middle for the Target plate and its mascot; a square Target plate was tried and reverted because at 720x1280 it pushed the merge path onto the table. Level Ready lost its target readout for a neutral mascot. The level map now scrolls - it had `MOUSE_FILTER_STOP`, so the ScrollContainer never saw the drag - and selection moved from press to release with a movement tolerance, since the first event of a flick is a press on whatever is under the finger. Decorative gems now fill the map's empty side, drawn from a slot-seeded RNG so the scatter never reshuffles mid-scroll.

- **Added the crowned-gem mascot as a mood dial.** The supplied contact sheet was sliced into two eight-frame tracks that share a neutral first pose. Callers never pick a frame: they set a mood and an intensity (`merge` 0.35, `combo` 0.55-0.9 by chain depth, `won` 1.0, `failed` 1.0, level start neutral) and `MascotView` glides to it. Two things make eight frames read as animation rather than a slideshow - the position along a track is a float that is *eased* toward its target so a jump travels through every pose between, and the two frames either side of that float are cross-faded. The lower frame is drawn opaque with the upper over it at the fractional alpha; drawing both semi-transparent is the obvious approach and is wrong, because the gold ring is identical in every frame and two half-opaque copies leave it 75% opaque, visibly dimming the mascot mid-blend. A mood flip mid-expression rewinds through neutral rather than cutting, since the two tracks only share that one pose.
- **Sliced the sheet with an elliptical mask rather than a grid crop.** The coins are pitched 206.4px apart and are ~206px wide, so neighbours touch and a square crop carried a crescent of the next coin's rim into every frame. Measured geometry (half-width 103, half-height 107) drives a 103x107 mask that keeps the raised hands in the late happy frames and removes the neighbours completely. Frames are cut on an evenly spaced grid, not on per-frame bounding boxes: a bounding box moves when the character raises its hands, and a crop following it would make the head jitter. `scripts/dev/slice_mascot_frames.gd` reproduces the 16 frames from the original on demand.
- **The mascot replaces the target readout in every popup.** The result screen showed the target gem on a win and a timer badge on a loss; neither said anything the title and reward card did not. It now carries the mascot at 300px - the expression is the point of the screen. The level briefing shows it neutral. It also sits in the centre of the top HUD between Coins and Next, reacting to play, and in the level-select header where the coin chip was.
- **Sized the HUD mascot to the coins card and top-aligned it.** The objective stack (SHOTS LEFT and the target card) is a separately anchored container that draws over the top row and whose vertical position moves with the table, so a mascot filling the band was hidden behind it on every limited-shots level.
- **Fixed the popup title plate animating separately from its popup.** The half-out gold banner is a *sibling* of the panel, and every popup tweened the panel - so the label appeared at full opacity before the popup arrived and outlived it on the way out. `UiDesignSystem.popup_shell()` now returns the banner and panel as one node, with the pivot set for scaling, and every popup animates that. The contract is enforced by a regression that asserts the panel is inside the shell and carries no animation of its own.
- **Made every popup wear the same title plate.** Daily Missions and the Powers shop already used the half-out banner; the result screen and the level briefing each had their own inline banner inside the body, and read as a different design language. All four now go through `popup_shell()`.
- **Added a daily reminder for unclaimed missions**, local and scheduled rather than push - the reminder is about state the device already knows, so it needs no server and works offline. It fires at 19:00 device-local **only when something is actually outstanding**: a reminder on a day the player already finished everything trains them to ignore the next one. The decision is re-evaluated on every mission-state change, so claiming the last mission at 18:55 cancels the 19:00 reminder, and an unopened chest counts as outstanding because it is still a reason to open the game. Every rule - whether, when, and what it says - is a pure function in `NotificationService`, so it is covered by tests rather than needing a device.
- **Added a REMINDERS toggle to Home settings**, persisted separately from the audio settings. This also fixed a latent bug: `save_settings()` rebuilt the config file from scratch and silently dropped any key it did not take as a parameter, so an audio toggle would have wiped the notification preference. It now preserves what it does not own.
- Added the Android plugin source for the native scheduler (`android/plugins/majestic_notifications/`): a Godot plugin singleton plus an alarm receiver. Inexact alarms on purpose - exact alarms need `SCHEDULE_EXACT_ALARM`, which Play grants only to alarm and timer apps, and a missions reminder does not need second accuracy. One stable notification id with `FLAG_UPDATE_CURRENT` so frequent reschedules replace rather than stack. **The plugin is not wired into the build and has never been compiled or run**; until it is, `NotificationService.is_available()` returns false and every call is a tested no-op. Wiring steps and the remaining gaps (no boot receiver, placeholder small icon) are in the plugin README.
- Added `tests/run_daily_reminder_mascot_v1_tests.gd` (reminder rules, timing across all 24 hours, copy, graceful degradation without the plugin, settings independence, frame registration, and that the mascot visits the middle of a track rather than cutting to the end) and `tests/capture_mascot_popups_v1.gd`. All 38 suites pass.

# 2026-09-05 - Level select map, per-level replay, milestone chests, and disk cleanup

- Exported and validated the release-signed companion APK `build/android/majestic-gems-level-select-map-v1.0.17-vc19.apk` from source commit `7328e95` (`level-select-map-v1-source`). It retains versionCode 19 / versionName 1.0.17, contains both supported ARM ABIs and the new compiled map scripts, and verifies under APK Signature Scheme v2 with the established upload certificate. No AAB was generated and no device was connected for installation or physical validation.
- **Added a level screen between Home and play.** The flow is now Home -> PLAY -> level map -> tap a level -> Level Ready popup -> play, and winning returns to the map rather than dropping straight into the next board, so the player always sees the path they are climbing. `LevelSelectOverlayLayer` (layer 61, between Home at 60 and the Level Ready popup Home itself owns) hosts the screen; `LevelMapView` draws the path.
- **Any cleared level can be replayed, and it rebuilds identically.** No seed storage was needed or added: `LevelConfig.seed_for_level()` is a pure function of the level number, so level 7 derives the same seed on every device and install, and `generated()` rebuilds the same opening board, target ladder, shot limit and launcher queue from it. `run_level_select_map_v1_tests` now asserts that contract directly across eight levels, because it is the assumption the whole replay feature rests on and nothing else in the suite covered it.
- **Split `highest_level` from `level_number`.** `level_number` is the level loaded on the table; `highest_level` is the furthest ever unlocked. Replaying level 5 out of 40 sets the first to 5 and leaves the second at 40, which is what keeps levels 6-40 unlocked and Home showing real progress. `save_progress()` takes the maximum against what is stored, so a replay can never walk the boundary backwards, and a session always opens on `highest_level` rather than on whichever earlier level the last session replayed.
- **The map draws 1000 levels beyond the player's furthest**, so scrolling never reaches an end and the game never looks finishable. Nodes, chests and the connecting path are drawn directly rather than built as child Controls: several thousand Controls would be laid out on every open, resize and scroll for a screen the player flicks through in a second. Only slots inside the visible window are drawn, so opening the map at level 4 and at level 4000 cost the same. Hit testing and drawing share one `_point_at()`, so what the player taps is always what they see.
- **Fixed a level resuming instead of restarting.** Leaving a level for Home and coming back replayed the half-finished table that was still loaded: `_show_home()` only hid the gameplay HUD and `_on_home_play_requested()` un-hid it, with nothing in between rebuilding the board. `_show_level_start()` now calls `restart()`, so every entry into a level builds it from scratch. The redundant second `restart()` in the skip-level path was removed.
- **Added milestone treasure chests every 20 levels.** A chest sits on the path after levels 20, 40, 60 and so on, unlocked by clearing the level below it. It pays 800 coins plus the daily chest's power grant (`switch` x2, `magnet`, `hammer`) and wears the same `badge_chest` / `badge_chest_open` art as the daily chest, so the two read as one reward object. 800 is roughly two levels' income at the measured 310-480 coins per level and exactly one Skip Level, which makes the payout legible against a price the player already knows - the daily chest's own `CHEST_REWARD` is 0 and was not a usable coin figure. Claims are granted with the daily chest's atomicity rule: the whole resulting inventory is built before anything persists, so a failed save cannot hand out half a chest.
- **The level screen is full-bleed.** The map fills the whole screen and the header banner and hero button float over it, pinned to the top and bottom edges. An earlier pass laid the three out as rows in a safe-area column, which inset the map into a panel in the middle of the screen and left both bars looking cramped. Two bugs came out of that rework and are worth recording: Godot's built-in `ScrollContainer` `panel` stylebox is a bordered grey plate and the project theme does not override it - that is what drew a visible box around the map; and `set_anchors_preset()` sets anchors but leaves every offset at zero, so both floating bars collapsed into the top-left corner until `_layout_bars()` drove their offsets from their own content height.
- **Removed the coin chip from the level screen.** Nothing on this screen spends coins, so the balance was a number the player could not act on. Home and the shop both still show it where it means something.
- The map is drawn from the supplied kit rather than from primitives: level plates are `btn_square_small` drawn through a StyleBoxTexture so the nine-patch rim keeps its authored thickness at every size, cleared levels wear `badge_check_laurel`, the current level is crowned with `badge_crown`, claimable chests glint with `icon_sparkle`, and `decor_diamond_small` studs are set into the path between nodes. The path itself is three strokes - dark casing, gold body, pale gloss - because a single flat stroke read as a drawn line rather than a paved road. The overlay sets `UiDesignSystem.theme()` on its own root: a CanvasLayer is not a Control, so without it every button falls back to Godot's default grey plate.
- Back is routed by the app-flow owner as before, with one addition: Back on Level Ready returns to the map that opened it rather than all the way to Home, so a player can pick a different level without a round trip. Home's single back signal is dispatched by flow state because the Home layer hosts both Home and the Level Ready popup.
- Added `tests/run_level_select_map_v1_tests.gd` (slot arithmetic and its inverse over 400 slots, chest unlock boundaries, the seed-purity contract, map geometry and hit testing, locked-level and locked-chest refusal, overlay presentation and centring, and the controller's routing) and `tests/capture_level_select_map_v1.gd`. Twelve existing suites gained the level-choice step now required between Home PLAY and play; `run_game_flow_reward_splash_tests` and `run_animation_audio_back_privacy_polish_tests` had their back-navigation assertions updated to the three-screen flow. All 38 suites pass.
- Removed 3.7 GB of build artefacts and report media: the gitignored `build/` tree (3.0 GB of AABs, APKs, bundletool output and logs) and the screenshot/video subdirectories under `reports/` (694 MB). All 158 report markdown files are kept, and the deleted report media remains in git history.
...
# 2026-09-05 - Supplied logos, 24-frame mascot, gradient splash, mobile scrolling, and reminders

- Replaced the transparent and illustrated-background logos with the newly supplied matching assets, preserving originals under `assets/logo/` and runtime roles separately.
- Re-sliced the new 12-by-2 transparent mascot sheet into 24 registered 160px runtime frames and retuned the mood glide for smoother intermediate motion.
- Replaced the solid engine boot presentation with a purple gradient and a medium centered transparent logo; removed the mascot from splash presentation.
- Fixed Android level-map scrolling by routing `InputEventScreenDrag` directly to the owning `ScrollContainer` without turning a swipe into a level tap.
- Wired the local daily-mission notification bridge into Android with `POST_NOTIFICATIONS`, Godot v2 plugin metadata, and the alarm receiver.

...
func sync_gems(pieces: Array[GemPiece]) -> void:
	...
	for piece in pieces:
		...
		visual.scale = Vector2.ONE * float(_presentation_scales.get(piece.id, 1.0)) * float(_impact_scales.get(piece.id, 1.0))
...
func set_impact_scale(piece_id: int, multiplier: float) -> void:
	_impact_scales[piece_id] = clampf(multiplier, 0.82, 1.12)

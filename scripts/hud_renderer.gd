...
static func _draw_queue_preview(controller: CanvasItem, rect: Rect2, level: int) -> void:
	...
	_draw_contained_texture(controller, entry.texture, rect.get_center() + Vector2(0.0, 19.0), Vector2(76.0, 66.0))
...
static func _draw_active_target(controller: CanvasItem, snapshot: Dictionary, font: Font) -> void:
	...
	controller.draw_texture_rect_region(AssetCatalogType.HUD_BUTTON_SHEET, GameConfig.TARGET_BODY_RECT, AssetCatalogType.HUD_WHITE_PANEL_REGION)
	...
	_draw_contained_texture(controller, entry.texture, GameConfig.TARGET_BODY_RECT.get_center(), GameConfig.TARGET_PREVIEW_BOUNDS)

static func _draw_restart_button(controller: CanvasItem) -> void:
	controller.draw_texture_rect_region(AssetCatalogType.HUD_RESTART_ART, GameConfig.RESTART_BUTTON_RECT, AssetCatalogType.HUD_RESTART_BUTTON_REGION)

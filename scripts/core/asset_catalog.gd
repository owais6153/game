...
static func gem_entry(level: int) -> Dictionary:
	...
	return {"tier": level, "identity": identity, "id": String(GEM_IDS.get(identity, "gem_unknown")), "name": "", "texture": gem_texture(level), "texture_path": gem_resource_path(level), "collision_radius": GameConfig.gem_collision_radius(level), "visual_scale": visual_scale(level)}

...
func begin_merge_feedback(merge_event: Dictionary) -> void:
	...
	merge_impacts.append({
		...
		"ring_layers": maxi(int(timeline.get("ring_layers", 1)), GameConfig.merge_vfx_wave_layers(result_level, target_merge)),
...
func _draw_merge_impact(effect: Dictionary) -> void:
	...
	for layer_index in range(ring_layers):
		# Each concentric front follows the previous one instead of occupying the
		# same radius. The result reads as a dense jewel shockwave without keeping
		# dozens of independently moving shard records alive.
		var delay := float(layer_index) * 0.055
		if t < delay:
			continue
		var wave_t := clampf((t - delay) / maxf(0.001, 1.0 - delay), 0.0, 1.0)
		var wave_expand := 1.0 - pow(1.0 - wave_t, 2.4)
		var wave_fade := (1.0 - wave_t) * fade
		var layer_t := float(layer_index) / float(maxi(1, ring_layers - 1))
		var layer_scale := lerpf(0.76, 1.22, layer_t)
		var layer_alpha := lerpf(0.92, 0.42, layer_t)
		var layer_color := color
		if target_merge and layer_index > 0:
			layer_color = Color(1.0, 0.88, 0.38, wave_fade * layer_alpha)
		else:
			layer_color.a = wave_fade * layer_alpha
		var width := lerpf((5.8 if target_merge or major_reward else 4.4) * tier_scale, 1.0, wave_t) * (1.0 - float(layer_index) * 0.08)
		draw_arc(center, (gem_radius * 0.56 + 50.0 * wave_expand) * effect_scale * layer_scale, 0.0, TAU, ring_segments, layer_color, width)

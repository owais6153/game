...
func _test_control_hierarchy_and_contained_previews() -> void:
	...
	_assert(hud.hud_margin.get_node("HudColumn/HudShell/HudRows") is VBoxContainer, "Gameplay header rows must use a VBoxContainer inside one production shell")
	_assert(hud.hud_margin.get_node("HudColumn/HudShell/HudRows/MainRow") is CenterContainer, "The integrated merge-path header must own the top row")
	_assert(hud.hud_margin.get_node("HudColumn/UtilityRow") is HBoxContainer, "COINS and NEXT must use one compact responsive utility row")

...
func _test_control_hierarchy_and_contained_previews() -> void:
	...
	hud.update_snapshot(_snapshot(125500, 2, 4, 5, 0, 1, false))
	await process_frame
	...
	_assert(hud.hud_margin.get_node("HudShell/HudRows") is VBoxContainer, "Gameplay HUD rows must use a VBoxContainer inside one production shell")
	_assert(hud.hud_margin.get_node("HudShell/HudRows/MainRow") is CenterContainer, "The integrated merge-path header must own the top row")
	_assert(hud.hud_margin.get_node("HudShell/HudRows/ScoreNextRow") is HBoxContainer, "COINS, target, and NEXT must use one responsive objective row")
	...
	_assert(hud.target_panel.get_parent() == hud.target_anchor, "The active target must own the expanding center slot")
	...
	_assert(hud.target_icon.texture == AssetCatalogType.gem_texture(5), "Target icon must match the first active L5 target")
	...
	_assert(hud.target_swap_outgoing.visible and hud.target_swap_incoming.visible, "A completed target must hold for confirmation before the centered old/new target crossfade")
	_assert(hud.target_swap_outgoing.position.distance_to(hud.target_swap_incoming.position) < 0.1, "Reference target handoff must fade in place without unrelated directional travel")
...
func _test_reference_coin_reward_path() -> void:
	...
	_assert(float(first_coin.scatter.y) < float(first_coin.start.y) and first_coin.scatter.distance_to(last_coin.scatter) < GameConfig.MAJOR_COIN_BURST_RADIUS * 1.25, "Reference coins must begin as one tight four-token cluster")
	_assert(is_equal_approx(float(first_coin.base_scale), float(last_coin.base_scale)) and first_coin.has("control_a") and first_coin.has("control_b"), "Coin tokens must retain one readable size language on the shared curved flight")
	var flight_ranks: Array[int] = []
	for coin in controller.effects_layer.coin_rewards:
		flight_ranks.append(int(coin.flight_rank))
	_assert(flight_ranks == [0, 1, 2, 3], "The four reference coins must depart in readable order with the final token last")
...
func _test_late_collection_fade_and_body_cleanup() -> void:
	...
	var target_events: Array[Dictionary] = [_merge_event(result_id, 5)]
	controller._apply_confirmed_merge_events(target_events)
	var physics_position: Vector2 = result.position
	var physics_radius: float = result.radius
	controller._update_merge_presentations(0.18)
	controller.gem_sprite_layer.sync_gems(controller.pieces)
	var presentation_visual: Node2D = controller.gem_sprite_layer._visual_containers[result_id]
	var presentation_root: Node2D = controller.gem_sprite_layer._piece_visual_roots[result_id]
	var presentation_scale: Vector2 = controller.gem_sprite_layer._presentation_scales[result_id]
	_assert(presentation_visual.position == Vector2.ZERO and is_zero_approx(presentation_visual.rotation) and is_equal_approx(presentation_scale.x, presentation_scale.y), "Merge result must keep a rigid silhouette and use only a centered uniform pop")
	_assert(result.position == physics_position and is_equal_approx(result.radius, physics_radius) and presentation_root.position == physics_position, "Merge animation must never alter simulation position, radius, or the physics-mirroring root")
	...
	_assert(GameConfig.TARGET_PANEL_PULSE_DURATION >= 0.88 and GameConfig.TARGET_PANEL_PULSE_DURATION <= 1.0, "Reference success check must remain readable through its rewarding hold")
	controller._refresh_hud()
	...
	_assert(controller.gameplay_ui.target_swap_outgoing.visible and controller.gameplay_ui.target_swap_incoming.visible, "L5 must hold under the success check before the centered L5-to-L7 fade handoff")
...
func _test_final_l8_event_order_and_single_overlay() -> void:
	...
	_assert(controller.target_index == 2 and not controller.win_qualified, "L7 must advance to L8 without starting a result overlay")
	var result_id := 7203

...
## Deterministic, development-only visual evidence for Production Gameplay UI
## V2. This script never joins the exported project or changes runtime rules.
...
const OUTPUT_DIR := "res://reports/production-gameplay-ui-v2/"
...
func _run() -> void:
	...
	paused = false
	print("PRODUCTION_GAMEPLAY_UI_V2_CAPTURE: PASS")
	quit(0)
...
func _new_fixture(physical_resolution: Vector2i) -> Dictionary:
	...
	viewport.name = "V2Capture_%dx%d" % [physical_resolution.x, physical_resolution.y]
...
func _capture(fixture: Dictionary, relative_path: String) -> void:
	...
	if error != OK:
		push_error("Unable to save V2 evidence %s (error %d)" % [destination, error])

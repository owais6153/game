...
func _test_hud_architecture_and_catalog_mapping() -> void:
	...
	_assert(hud.hud_margin.get_node("HudColumn") is VBoxContainer, "HUD header and utilities must use one responsive top column")
...
func _test_score_fit_and_state_updates() -> void:
	...
	for score in scores:
		...
		_assert(hud.score_label.get_global_rect().end.y <= hud.score_panel.get_global_rect().end.y - 8.0, "Coin total %s must retain visible bottom breathing room" % str(score))

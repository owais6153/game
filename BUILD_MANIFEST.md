...
- Size: 83,450,842 bytes
- Built: 2026-09-09 13:40:36 +05:00
- SHA-256: `A5DDD4CD4646F84EB6978957A47F2BE755BDD5A9B83A8400A0DA4F10E2871866`
- Source commit/tag: `609d144` / `level-stars-v1`; the tree was clean at export.
- Rebuilt from an earlier 13:09 export of the same version pair after the supplied star art and award audio landed. The earlier artifact was never delivered; this is the one recorded.
...
- Contents: level stars (three solver-derived objectives per level, sequential award, per-level and total display) drawn from the supplied star art, per-level record pinning, the fullscreen treasure ceremony with post-win drops, and the supplied award/treasure audio cues.
...
- Size: 83,588,090 bytes
- Built: 2026-09-09 13:41:43 +05:00
- SHA-256: `1283F86E529C45AB7B7D5D1792739400D5775080B2FF95C4651C5F4D038A96E9`
- Source commit/tag: `609d144` / `level-stars-v1`; the tree was clean at export.
...
- **Device-verified in this build**: the supplied star art renders throughout - the level-screen header carries the running total alone (`★ 8`, no disc, no chest countdown), cleared level nodes render their own three-star rows in the same art (levels 3 and 5 showed `★★☆`), and a level advanced by Skip Level correctly shows three empty outlines, since skipping is not a win and awards no stars.
...
- Not device-verified: the star award sequence and its supplied audio on hardware. The sequence, the focus dim and the settled row are proved in the `reports/level-stars-v1/` renders and by regression; nothing has played the ding or the shine through a speaker, and no device play-through to a win was performed. Also unverified: Firebase DebugView, live ad playback, notification delivery.
...
- **First export attempt failed and produced no artifact.** Gradle reported `Gradle build daemon disappeared unexpectedly`; the daemon had been alive for 1h12m and had survived an earlier run that exhausted disk space on `D:`. Note that **Godot returned exit code 0 for that failed export** - its exit status is not a reliable success signal, so artifact existence plus Bundletool version validation is the gate. Stopping all daemons (`gradlew --stop`) and re-running produced this artifact.
- **The successful export also did not exit cleanly.** Godot completed the export (`[ DONE ] export`, no `FAILURE` block, no errors in the log) and wrote the full artifact, then hung on shutdown and was killed by the 2400 s `timeout` wrapper - exit code 124. This is the same class as the known Windows post-run shutdown fault recorded against earlier builds. The artifact is unaffected and was proven complete independently of the exit status: `unzip -t` reports no CRC errors across the whole archive, Bundletool `validate` passes, `jarsigner -verify` verifies every entry, and the SHA-256 was re-computed after the process ended and is byte-identical to the value recorded above. Taken together with the failed first attempt returning 0, the rule for this project is that **Godot's exit code carries no information about export success in either direction** - validate the artifact.
...
- Companion release APK: `build/android/majestic-gems-release-v1.0.17-vc19.apk`, 76,342,851 bytes; 2026-09-03 11:05 +05:00; SHA-256 `8B38DCE04573A4E53A773D93DA7E95876337FA0EE4794A43553F4AED3512EA48`. Same identity as the AAB - aapt2 badging reports package `com.owais.majestygems`, versionCode 19, versionName 1.0.17, target/compile SDK 36, `arm64-v8a` + `armeabi-v7a` - and signed with the same upload key: `apksigner verify --print-certs` reports `CN=Muhammad Owais Khan, OU=Development, O=Teckvertex Labs` and verifies under APK Signature Scheme v2. `unzip -t` reports no CRC errors, and the new analytics events and template ids were grepped out of its packaged bytecode. This is for sideloading and device testing; the AAB is the Play artifact.
- APK export note: the first attempt failed with "Please provide a valid project path when exporting" because the preset name `Android APK` contains a space and Godot mis-parsed the argument list. Passing `--path "d:/Owais/game"` explicitly fixes it. The AAB preset (`Android`, one word) does not hit this.
...
- Companion release APK: `build/android/majestic-gems-release-v1.0.16-vc18.apk`, 76,316,479 bytes; 2026-08-31 07:47:58 +05:00; SHA-256 `69EE32AA9250B82A9D3E4991E2D15E1767A91809D0F2E1932447276A9E23DEB2`. Same identity as the AAB (aapt2 badging reports versionCode 18, versionName 1.0.16, `arm64-v8a` + `armeabi-v7a`) and signed with the same upload key - `apksigner verify --print-certs` reports `CN=Muhammad Owais Khan, OU=Development, O=Teckvertex Labs` and verifies under APK Signature Scheme v2. This is for sideloading and device testing; the AAB is the Play artifact.

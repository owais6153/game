...
Dynamic gems must breathe inside their panels. The target icon never breaks the cream card boundary; its name and labeled progress share a left edge; the progress track remains visible at zero. Coral carries objective/state emphasis, teal utility/path labeling, gold connections and edge accents, cream information surfaces, and warm brown primary values. Shadows are short and consistent. The danger line uses coral over a translucent dark backing so it remains readable on pale felt without becoming a new gameplay effect.
...
Visual-physics calibration v1 uses a shallower derived coral-table perspective so the tabletop reads as part of the tropical camera rather than a separate steep viewpoint. The table remains contained and leaves the environment visible around it.

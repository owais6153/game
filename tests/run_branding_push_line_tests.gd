...
func _test_brand_assets() -> void:
	var logo := (load("res://assets/runtime/ui/majestic_gems_logo_v4.png") as Texture2D).get_image()
	var legacy_icon := (load("res://assets/runtime/ui/majestic_gems_app_icon_192_v5.png") as Texture2D).get_image()
	var adaptive_foreground := (load("res://assets/runtime/ui/majestic_gems_adaptive_foreground_v5.png") as Texture2D).get_image()
	var adaptive_background := (load("res://assets/runtime/ui/majestic_gems_adaptive_background_v5.png") as Texture2D).get_image()
	var opaque_source := Image.load_from_file(ProjectSettings.globalize_path("res://assets/logo/majestic_gems_logo_with_background_source_v5.png"))
	_assert(logo.get_size() == Vector2i(1536, 1024) and logo.get_pixel(0, 0).a == 0.0, "Transparent Home logo v4 must remain unchanged")
	_assert(opaque_source != null and opaque_source.get_size() == Vector2i(512, 512) and opaque_source.get_pixel(0, 0).a == 1.0, "Supplied opaque background logo must be preserved in the semantic source folder")

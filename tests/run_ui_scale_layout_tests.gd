...
func _test_feedback_polish_contract() -> void:
	_assert(GameConfig.MERGE_PRESENTATION_DURATION >= 0.45 and GameConfig.MERGE_PRESENTATION_DURATION <= 0.60, "Merge presentation must complete inside the readable 450-600 ms window")
	_assert(GameConfig.MERGE_SOURCE_PULL_DURATION >= 0.18 and GameConfig.MERGE_SOURCE_PULL_DURATION <= 0.22, "Merge source combination must remain readable")
	_assert(GameConfig.MERGE_RESULT_POP_SCALE >= 1.12 and GameConfig.MERGE_RESULT_POP_SCALE <= 1.20, "Merge result overshoot must be visible and controlled")
	...
	_assert(GameConfig.TARGET_COLLECTION_DURATION >= 0.60 and GameConfig.TARGET_COLLECTION_DURATION <= 0.80, "Target collection must remain readable")
	_assert(GameConfig.COLLISION_VISUAL_DURATION >= 0.10 and GameConfig.COLLISION_VISUAL_DURATION <= 0.18, "Collision micro-feedback must remain short")

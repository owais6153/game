...
func _test_feedback_contracts() -> void:
	_assert(GameConfig.MERGE_RESULT_POP_SCALE == 1.26 and GameConfig.MERGE_PRESENTATION_DURATION == 0.27, "Merge pop must retain the tester-approved fast feel")
	_assert(GameConfig.COIN_FLIGHT_DURATION == 0.54 and GameConfig.MAJOR_COIN_FLIGHT_DURATION == 0.60 and GameConfig.COIN_FLIGHT_STAGGER == 0.045, "Coin flights must retain the tester-approved cadence")
	_assert(GameConfig.TARGET_COLLECTION_DURATION == 0.32, "Target collection must retain the tester-approved cadence")

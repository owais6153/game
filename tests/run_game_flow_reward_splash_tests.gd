...
func _test_production_controller_home_flow() -> void:
	...
	_assert(controller.app_flow_state == controller.AppFlowState.LEVEL_SELECT and controller.level_select.is_open(), "Home PLAY must open the level map without starting gameplay")
	controller._on_level_chosen(controller.highest_level)
	_assert(controller.app_flow_state == controller.AppFlowState.LEVEL_READY and controller.home_overlay.level_intro_blocker.visible, "Choosing a level must enter Level Ready without starting gameplay")
...
func _test_back_and_idle_state_ownership() -> void:
	...
	_assert(controller._handle_back_request(false) == "exit_confirmation" and controller.home_overlay.exit_confirm_blocker.visible, "Back on the bare Home screen must open exit confirmation instead of terminating Android")
	_assert(controller.app_flow_state == controller.AppFlowState.HOME and paused and not controller.gameplay_ui.is_pause_visible(), "A Home exit confirmation must never open gameplay Pause or unpause the hidden board")
	_assert(controller._handle_back_request(false) == "home_overlay" and not controller.home_overlay.exit_confirm_blocker.visible, "Back on exit confirmation must close only that topmost popup")
	controller._on_home_level_intro_requested()
	_assert(controller._handle_back_request(false) == "home", "Back on the level map must be consumed by the app flow owner")
	_assert(controller.app_flow_state == controller.AppFlowState.HOME and controller.home_overlay.home_backdrop.visible, "Back on the level map must return to Home")
	controller._on_home_level_intro_requested()
	controller._on_level_chosen(controller.highest_level)
	_assert(controller._handle_back_request(false) == "home_overlay", "Back on Level Ready must be consumed by the Home overlay")
	_assert(controller.app_flow_state == controller.AppFlowState.LEVEL_SELECT and controller.level_select.is_open(), "Back on Level Ready must return to the level map that opened it, not all the way to Home")
	controller._show_home()
	controller._on_home_level_intro_requested()
	controller._on_level_chosen(controller.highest_level)
	controller._on_home_play_requested()
...
func _test_controller_flow_guards() -> void:
	...
	_assert(export_source.contains("splash_screen/icon=\"res://assets/runtime/ui/majestic_gems_system_splash_1152_v6.png\""), "Android launch splash must use the supplied-background v6 derivative")
	...
	var splash := Image.load_from_file(ProjectSettings.globalize_path("res://assets/runtime/ui/majestic_gems_system_splash_1152_v6.png"))
	...
	_assert(GameConfig.target_coin_reward_for_result_level(2) == 2 and GameConfig.target_coin_reward_for_result_level(3) == 6 and GameConfig.target_coin_reward_for_result_level(4) == 14 and GameConfig.target_coin_reward_for_result_level(5) == 30 and GameConfig.target_coin_reward_for_result_level(6) == 55 and GameConfig.target_coin_reward_for_result_level(7) == 120 and GameConfig.target_coin_reward_for_result_level(8) == 260, "Target coin reward table must remain explicit and auditable")

...
**Current milestone:** Identity, UI, Unlimited Play & Target Balance Fix v1. `AssetCatalog.gem_entry` is the authoritative identity source for all 18 names and runtime/HUD textures. Level 1 has unlimited launches and sequential Jade x2 (L3), then Aquamarine x2 (L4) targets. A collected target is removed from `pieces` before its HUD animation, so it cannot become an invisible collision or merge body. The score/progression/next HUD follows `assets/ui/Generated image 1 (3).png`; table, rails, perspective scaling, colliders, motion, and contact merging are unchanged. APK: `build/android/identity-ui-unlimited-target-balance-fix-v1.apk`. See `reports/IDENTITY_UI_UNLIMITED_TARGET_BALANCE_FIX_V1_REPORT.md`.

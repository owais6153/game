...
- **The stars are the supplied art now, not drawn polygons.** One supplied sheet, sliced by `scripts/dev/prepare_star_kit_art.gd` into `star_filled`, `star_empty` and `star_glow`. Slice rects were measured by walking the sheet's alpha columns, not eyeballed, and the sheet size is asserted before they are used. The result popup, the level map and the level-screen header all draw the same three textures, so a level's stars look the same wherever they appear.
- **These derivatives are deliberately not hard-cut or eroded.** Those passes exist to strip the red fringe the power sheets carry; this source has none, and the glow variant is semi-transparent across most of its area - a 0.5 alpha cut would delete the halo and an erosion pass would chew the anti-aliased edge off every star. Only sub-threshold noise is cleared, at the 0.01 the gem derivatives use.
- **A star is not square, so nothing treats it as square.** Every consumer sizes by height and derives width from the texture's own aspect; the row's slot pitch follows it too, or three stars would overlap.
- **The award is heavier and the screen gets out of its way.** Each star grows from 0.10 to a 1.62 overshoot, bounces once, and settles on exactly its placeholder; the supplied glow star blooms behind it at 1.85x and fades over the settle; a ring and ten rays fire at the peak. While the sequence runs, every other element on the popup - subtitle, coin figures, transition line, all three buttons - fades to 34% so the row is unambiguously what the screen is doing, then restores. Held back rather than hidden, because hiding would reflow the column and move the row mid-sequence.
- **The award cues are the supplied recordings.** `star_award` is the supplied ding and `treasure_claim` is the supplied shine, both asserted by identity in `run_sound_privacy_link_tests` so a later refactor cannot quietly swap them back for procedural chimes. The star ladder is an explicit `[1.0, 1.12, 1.25]` rather than a base and a step - the three values are the tuning, and a formula only hid them.
- All 40 suites pass.

# Star Row Sequencing and Header Cleanup - 1.0.19 (vc21) - 2026-09-09

- **Stars fill left to right, and this reverses an earlier decision.** The row lit the *specific* objectives that were met, so earning objectives 1 and 3 left a gap in the middle. That is more precise and worse: a player who earned two stars expects to see two, together, and it disagreed with the map and the header, which are both counts. The row is a score out of three now; which objectives were met is reported by the caption, which names each earned objective as its star lands.
- **The award grows out of its placeholder instead of flying in.** A star that arrives from off its slot has to be tracked by the eye before it can be read, and three of them in a row read as busy rather than as earned. Each star now grows from its own empty placeholder, overshoots to 1.34, and settles back onto exactly the placeholder's size and position. No travel, no spin.
- **The filled disc behind each star is gone.** It read as a badge the star was sitting on rather than as light coming off it. The ring and the short rays remain, and they fire at the peak of the growth rather than during it, so the impact reads as the star landing on its slot.
- **The level-screen header carries the star total alone.** The star is smaller (26 -> 20px) and the "next chest in N levels" countdown is removed: it was permanent, moved by one per level, and named something the player can do nothing about - the chest is drawn on the path where they will reach it. The one actionable case, a chest earned and still unopened, is still announced, and the separator dot only appears when there is something after it.
- All 40 suites pass.

# Star Award Polish, Treasure Pacing, and Level Ready Hierarchy - 1.0.19 (vc21) - 2026-09-09

- **Fixed the star row lighting stars the player did not earn.** `StarRow.award()` raised a lit *count*, so awarding the third star of a `[earned, missed, earned]` result lit all three while the caption underneath read "2 of 3". A count cannot express a gap and a gap is the normal case; lit state is per star now, and `lit_count()`/`is_lit()` are derived from it so the row can never disagree with the tally.
- **The row no longer pre-lights the stars the player already held.** Storage is a count, not a set, so "two stars already" cannot say *which* two. The row starts empty and lights exactly what this attempt earned.
- **The award is a real award now.** Each star drops in from 2.9x scale with a spin, lands with an overshoot, a white bloom, an expanding ring and a ten-ray burst, and a settled row keeps a slow shimmer. Stars went from 62px to 78px. Full marks reads `PERFECT!` with a kick rather than `3 of 3 stars`.
- **Stars and treasure claims have their own cues, and both rise.** `star_award`, `star_complete` and `treasure_claim` are new chime identities in `GameConfig.AUDIO_TONES`; the controller plays them at `GameConfig.star_award_pitch(index)` so a sequence reads as one climbing phrase rather than the same note repeated. The audio-service cache bound moved 25 -> 28, which is what caught the additions.
- **The treasure fanfare moved to the moment the lid gives.** It was played on `present()`, a second early, over a chest that was still shut and still waiting to be tapped. `chest_opened` and `reward_claimed` are new presentation signals; the controller owns the cues, so no UI layer talks to the audio service.
- **Bonus treasures can no longer land back to back.** The raw 15% roll averages correctly over hundreds of levels but a hash is only uniform in the large: it fired on levels 1, 2 and 5, and five times in the first twenty - exactly the levels a new player sees, which read as a treasure every level. An accepted drop now also requires the previous two levels to have failed the roll, so drops are at least three levels apart and the effective rate is 10.2%. Still a pure function of the level number; replays are unchanged.
- **Level Ready puts the mascot back as the focus.** The objectives were above it in a bordered card, which made them the first thing the eye landed on and pushed the mascot down the panel. They sit below the mascot now as a plain list with no frame - a second panel inside a popup was competing with the popup.
- **Fixed a statically lit star drawing at the full arrival scale.** `_draw` reads `_pop` as arrival progress and `resize()` zero-fills, and zero means mid-arrival - so every star that was never animated (the header total, the map rows, the level-start list) drew at 2.9x. 1.0 is settled, and that is now the default. The row's measured box no longer reserves burst room either; `clip_contents` is off on Control and on every container these rows sit in, so the ring draws past the bounds anyway.
- **HOME is no longer gated on the star sequence.** A proof capture caught it rendering exactly as it does when live while refusing taps, because its plate carries no distinct disabled art. It is an escape hatch rather than a reward action; COLLECT and DOUBLE COINS remain gated.

# Level Stars and Pinned Level Records - 1.0.19 (vc21) - 2026-09-09

- **Every level now has three star objectives**, resolved by `LevelStars` from that level's own generated config: complete it, finish inside a shot budget, and one of three rotating objectives (merge N gems, reach combo N, win without a power). The same array feeds Level Ready and the result popup, so the objectives a player is shown are word-for-word the ones they are judged against.
- **Every numeric threshold is derived from the level's own play-out, never authored.** `LevelSolver.simulate()` now reports merges as well as shots, and both numeric stars are sized against it. This is the direct lesson of the hand-written shot ladder that made every limited level unwinnable: a threshold that is not tied to what the level contains drifts silently the moment a template is retuned.
- **The measured headroom is 25% at its tightest.** Across levels 1-120 the shot star never asks for less than a 25% margin over a perfect play-out, and on a limited level it always sits strictly inside that level's own budget. `scripts/dev/print_level_star_audit.gd` prints the table; `run_level_stars_v1_tests` asserts the bounds for all 120.
- **The merge star is deliberately the most forgiving of the three, and sits *below* a perfect play-out.** A real player takes 30-70% more shots than the play-out but also strands gems it would have paired, and the net of those two is not knowable from the config. The failure modes are not symmetrical - a slightly easy star is a mild waste, a star nobody can reach is a promise the level-start screen broke - so the bar is 90% of the demonstrated count, rounded down to a multiple of five so it reads as an objective rather than as a formula output.
- **Stars are banked before the popup opens.** Evaluation, persistence and the analytics event all happen in `_award_level_stars()` at win time; the popup animates a record. Storage is monotonic per level, so going back for a missed star can only ever help.
- **COLLECT and DOUBLE COINS are gated on the star sequence**, and stay disabled until the last star has landed - a fast tap on the popup used to be enough to skip an award the player never saw. HOME is deliberately not gated: a proof capture caught it rendering exactly as it does when live while refusing taps, because its plate has no distinct disabled art. A loss shows no star row and gates nothing.
- **A level the player has entered is pinned to its own snapshot.** Generation was already pure and still is - `run_level_select_map_v1_tests` asserts that directly - but purity is with respect to *our generator*, and a template retune or a `GENERATOR_VERSION` bump silently rewrites every level already played. `LevelRecordService` snapshots seed, gem identities, opening board, queue, targets, shot limit, background and table on first entry and restores them afterwards. A pinned level is never re-pinned.
- **`LevelRecordService.matches()` compares structurally, not as strings.** A ConfigFile round trip does not preserve Dictionary key order, so comparing `str()` of two identical opening boards reported a mismatch on every level and would have made the drift detector cry wolf permanently.
- **Records live in `user://level_records.cfg`, not in the progression save.** The progress file is read on every launch and written on every coin transaction; a player at level 300 would otherwise be parsing and rewriting three hundred board layouts to bank one merge.
- All 40 suites pass, verified over two consecutive full passes.

# Treasure Ceremony V1 - 2026-09-09

- **Every treasure now opens in one fullscreen ceremony**, `TreasureOverlayLayer` on layer 70. The daily chest, the milestone chest and the new post-win bonus chest all route through it, so a treasure looks and feels the same wherever the player meets one. Before this the daily chest opened as an 88px icon inside a popup and the milestone chest paid out with no animation at all - the largest reward in the game was the least noticeable thing on screen.
- **A treasure opens on a tap and pays out on taps.** The chest waits under `TAP TO OPEN`; each reward then arrives alone under `TAP TO CLAIM` and leaves only when tapped. Nothing advances on a timer. An auto-advancing reveal was the obvious design and is worse: the player's eye is still on the coin pile when the powers flash past.
- **A level win can drop a treasure, before the win popup rather than after it.** A twentieth level hands over its milestone chest immediately; every other win rolls a 15% bonus of 120 coins plus one power. `TreasureDrop` is a pure function of the level number for the same reason `seed_for_level` is - a replay of level 20 must behave like level 20.
- **The level's own reward is captured before the treasure is granted.** A milestone pays 800 coins into the same balance Level Complete reads, so without this the popup would claim the level paid 900 and Double Coins would offer to match it. The treasure moves `level_start_coins` with `coins`, keeping `coins - level_start_coins` equal to what the level itself paid.
- **The reward is persisted before the ceremony starts, never during it.** The whole resulting inventory, balance and chest record are built and saved first, then adopted - the same atomicity rule the daily and milestone chests already used. A player who force-closes the app mid-animation keeps everything.
- **The chest art is its own tap target.** The daily popup's chest carries an invisible button with every state styled empty. `flat = true` only suppresses the draw call; the button would still have inherited the shared 92px gem plate and been measured against it by `run_ui_kit_polish_v1_tests`, which an 88px chest cannot carry without crushing its caps.
- **The win popup's mascot is unaffected and is asserted, not assumed.** The mascot frames are a static cache shared by every instance and the ceremony runs its own always-processing layer with its own tweens, so `run_treasure_ceremony_v1_tests` drives a full ceremony and then checks the popup still travels its happy track.
- **Two existing suites needed updating because the behaviour genuinely changed.** `run_reward_feedback_v3_tests` measures the level's own coin reward end to end and was driving its win on whichever level the save happened to hold; it now pins an odd, treasure-free level, since a post-win treasure is a separate reward in the same balance and would hold the popup it waits for.
- All 39 suites pass, verified over two consecutive full passes.
- **No Android artifact was produced for this change.** The APKs and AABs in `build/android/` are the 1.0.18 (vc20) release and predate it.
...
- **Replaying a cleared level rebuilds it exactly.** Nothing stores per-level seeds and nothing needs to: `LevelConfig.seed_for_level()` is `(n * 1103515245 + 12345) & 0x7fffffff`, a pure function of the level number, and every generated property follows from it. A replay of level 7 is bit-identical to the first play on any device or install. `run_level_select_map_v1_tests` asserts this directly - if the function ever stops being pure, replays silently diverge and no other suite would notice.
- **`highest_level` and `level_number` are different things.** `level_number` is the level on the table; `highest_level` is the furthest ever unlocked. Replaying level 5 out of 40 leaves `highest_level` at 40. `save_progress()` only ever raises it (it takes the maximum against the stored value), so no replay path can re-lock the map. A cold start opens on `highest_level`, not on the last level replayed.
- **The map shows 1000 levels past the player's furthest**, so it never presents an end. It is drawn, not built from Controls - only the visible slots are touched, so the map costs the same at level 4 and at level 4000. Drawing and hit testing share one geometry function, so taps always land where the plates look.
- **Leaving a level and coming back now restarts it.** It used to resume the half-played table, because Home only hid the HUD and nothing rebuilt the board in between. `_show_level_start()` calls `restart()`, so every entry builds the level from scratch.
- **A treasure chest sits on the path after every 20th level**, unlocked by clearing the level below it, paying 800 coins plus the daily chest's power grant and wearing the same chest art. Claims persist atomically the same way daily chest claims do. 800 coins is about two levels' income and exactly one Skip Level; the daily chest's own `CHEST_REWARD` is 0, so it was not a usable coin figure to copy.
- Back on Level Ready returns to the map rather than to Home, so a player can change their mind about which level to play without a round trip.
...
- **The shop coin desync is fixed.** The game keeps two balances: `coins` (display, includes the attempt's unresolved target earnings, rolled back by `restart()`) and `level_start_coins` (banked, what every sink spends). The shop displayed the first and the purchase checked the second, then returned a bare `false` the caller dropped - so Buy did nothing and no message appeared. The daily-mission claim made the gap permanent by crediting only `coins`. Banked is now the single authority behind `spendable_coins()`; the shop reads and live-refreshes from it, a refused Buy names its reason on screen and in analytics, and a lock prevents double-tap double-purchase. The same display-vs-banked mistake in the extra-shots and continue paths (which could drive the bank negative) is fixed too.
- **The shooter now follows a drag anywhere on the table below the danger line**, not only on the gem or the narrow aim-guide strip. The aim-guide grab above the line is unchanged, and popups/HUD controls still consume their own presses before `_unhandled_input`, so no UI tap can be stolen.
- **Levels are composed from 18 reusable templates** instead of three independent ladders that all capped out by level 26 and left two repeating shapes. A template names queue band, opening board archetype and density, target structure, and limited-shot margin together, so difficulty is a property of the combination. 11 layout archetypes, 10 target structures, 5 queue bands - all driving systems that already shipped. No new mechanics.
- Limited-shot cadence moved off "every third level" to an irregular seven-in-twenty-three pattern (gaps of 2-5, no value dominating). 23 is prime, so it shares no factor with the 8-long pacing cycle and the two together do not repeat for 184 levels. An earlier 13-length pattern was rejected because its gaps settled into a learnable 2,5,2,4 loop.
- Verified over levels 1-100: all 18 templates used, zero consecutive repeats, zero invalid configurations. Coin income per level is 12.3% below 1.0.16 (310/412/470/480/452 against 425/493/518/493/493) because ladders are shorter - but levels demand 12.6% less material, so the earn rate per unit of merge work is unchanged at +0.3%. Coins per minute is flat and the sink prices stay calibrated; see ECONOMY.md. `scripts/dev/print_level_template_audit.gd` and `scripts/dev/verify_level_systems.gd` reproduce this.
- **Analytics can now measure real-user difficulty per composition.** Every level event carries template/layout/band/queue/target-structure/generator-version; the three attempt-ending events carry a per-attempt aggregate (shots, merges, chains, max chain depth, combo 1/2/3+, merge and chain rates, targets reached, powers used and which most). One event per attempt, not one per shot. Everything is budgeted against GA4's 25-parameter ceiling, which silently drops overflow - the service warns and the suite asserts it.
- `level_abandon` added and routed through the same once-only latch as completion and failure, so an attempt yields exactly one outcome event. Shop/skip/continue/extra-shot funnels, `final_target_complete`, `daily_missions_open`, and `coin_source`/`coin_sink` added. Twelve events renamed to one vocabulary, each keeping its old parameters so saved reports resolve; reports spanning the 1.0.16/1.0.17 boundary must union old and new names.
- `NEXT_GEM_REROLL_COST` removed - a documented 100-coin sink no shipped path ever charged. Switching the queued gem is a power spent from inventory.
- **`ECONOMY.md` was badly stale and is corrected**, with the corrections recorded rather than overwritten: target rewards were documented as `10..1800` against a shipping `2..260`, Skip appeared as both 200 and 800 coins, the daily chest was described as paying 180 coins when `CHEST_REWARD` is 0, and the file claimed no shop exists.
- New docs: `LEVEL_DESIGN.md` (template system, and the audit of sequential-target counting - a gem merged above the active card is never banked for a later one, which is preserved and which the templates account for by keeping tiers ascending) and `reports/DIFFICULTY_ANALYTICS_GUIDE.md` (which events to inspect and what the numbers mean for too easy, too hard, boring, good challenge).
- **Note for future balancing:** `LevelSolver` banks pre-built gems and the real game does not, so every solver figure is an upper bound on real play. Treat it as a feasibility floor, never a difficulty prediction.
- All 36 regression suites pass, including two new ones. Five existing suites were updated where they asserted the old design (monotonic rows, fixed limited targets, monotonic margins, pre-rename event names); `run_rescue_softlock_blast_safety_v1_tests` zeroed only the display balance to mean "no coins" and had been passing only because it never ran after a test that banked coins.

# Release Packaging, Launch Screen, and 1.0.16 (vc18) - 2026-08-31

- The release AAB is `build/android/majestic-gems-release-v1.0.16-vc18.aab`, versionCode 18 / versionName 1.0.16, signed with the real upload key and version-validated with Bundletool. 18 is strictly above every code in `BUILD_MANIFEST.md` (highest prior 17); the last Play upload was 12 / 1.0.10.
- Per-device download is 49.8 MB on arm64-v8a, 51.4 MB on armeabi-v7a, measured with Bundletool on a split APK set. 26.0 MB of packaged asset data was removed by excluding source art no reachable code loads. `assets/runtime/ui/kit/` is deliberately still packaged in full because it is loaded dynamically by name.
- The blue launch screen is gone. `screen/background_color` was `#095579` and drove `android:windowBackground` plus the baked `--background_color` launch argument; it, the Android splash colour, and the boot splash colour are now `#1C0734`, matching the adaptive launcher-icon background so the launch reads as one continuous deep purple.
- Device validation of this build is outstanding: the phone went offline during install and did not recover over adb, so the launch sequence was verified only in the generated theme and the bundle resource table, not on screen.

# Low-End Performance, Rescue Softlock, and Merge Presentation - 2026-08-31

- GPU texture memory on the reported device (vivo Y21A / V2149, Helio P22, PowerVR GE8320, 3806 MB RAM) drops from 139.7 MB to 79.3 MB at Home and 143.3 MB to 82.9 MB in play - a ~60 MB (42%) reduction, measured on both builds under one identical cold-start protocol against a baseline exported from HEAD. Total PSS moves less (404 -> 390 MB) because it is dominated by code and the ad SDK, and the ~79 MB that remains is outside the scene-art path (ad SDK and EGL surfaces), which was not investigated. All ten backgrounds and all ten tables were `preload`ed into `const` arrays; scene art is now path-addressed behind a two-entry MRU cache and imports VRAM-compressed. The device advertises `GL_KHR_texture_compression_astc_ldr` and OpenGL ES 3.2, so ETC2/ASTC is safe there.
- Crowded-board simulation is 4.0x-6.4x cheaper. The pair sweep was O(N^2) across seven separation passes and up to eight substeps - roughly 43,000 `_resolve_pair` calls per frame at 40 gems - and `_resolve_bounds` re-derived the same rail geometry through `GameConfig` on every call. A vertical sweep-and-prune broad phase, frame-cached geometry, and an early exit when a sweep changes nothing replace that. The board is byte-identical to the previous simulation and a regression enforces it against a preserved baseline.
- On-device frame times through real gameplay at 17-20 gems: median 18.06 ms, p90 19.44 ms, p99 22.67 ms, worst ~30 ms, no stalls. Typical play sits at 10-15 gems where the median is a locked 16.67 ms.
- The level-entry table reveal is off. The briefing popup already shows the table behind it, so fading it in on START GAME read as the table vanishing and returning.
- The out-of-shots rescue popup can no longer softlock. Tapping buy disables every button including GIVE UP; a declined purchase now re-enables them, and a player who cannot pay is offered a rewarded video instead of a dead "NOT ENOUGH COINS". The same dead end on CONTINUE is fixed identically. With no ad fill the offer states the price and closes cleanly.
- The bomb no longer destroys objective gems. Tiers above `POWER_BOMB_MAX_CLEARED_TIER` (4) are pushed by the blast but never removed.
- The merge burst was never visible: its sprites used `z_index = -4096`, which is relative to the parent layer, so they resolved below the table sprite and were drawn behind opaque table art. Every merge the player has ever seen was the thin effects-layer ring arcs alone. The burst now draws above the gems, where its additive blend lights the result. On top of that fix it is one additive quad carrying a crisp expanding ring with the rotation riding on the ring, sized at 1.05x the gem and timed at 0.30s to match the merge cue, which peaks at 0.10-0.15s and is ~20 dB down by 0.30s. Each colour family has its own arm count and rotation direction; objective tiers gain a brighter accent and denser burst.
- All 34 regression suites pass, including three new ones (broad-phase equivalence, rescue softlock and blast safety, merge burst presentation). Three existing suites were corrected: two sampled pixels from now-compressed table art, and the gem-pattern suite asserted three targets on every level when limited-shot levels deliberately have two - that one was already failing before this work.
- Performance telemetry ships disabled (`GameConfig.PERFORMANCE_TELEMETRY_ENABLED`); it was enabled only for the measurement builds.

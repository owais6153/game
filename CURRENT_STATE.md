...
This milestone begins from clean `level-1-balance-v1` commit `9e03352`. It changes only the shared table/view composition and gem presentation depth: lower table landmarks, bounded visual-only Y perspective, and stable front/back ordering. Level 1 targets/balance, launcher queue/weights, gem order, collision radii, merge rules, score, danger failure, sounds, haptics, and all tier intrinsic sizes remain unchanged.

...
`UiDesignSystem.popup_shell(title, panel)` returns a column holding the half-out
gold title banner and the panel as siblings. The banner deliberately overlaps
the panel's top edge, which is why it cannot be a child of it.

That structure has one consequence that must not be rediscovered: **the panel is
not the popup**. Every popup used to tween `panel.scale` and `panel.modulate`,
which moved the body and left the title plate sitting at full opacity - the
label arrived before the popup and outlived it. The shell is the animation
target, its pivot is maintained on resize, and
`run_ui_kit_polish_v1_tests` asserts the panel carries no animation of its own.

All four popups - result, level briefing, daily missions, powers shop - are built
this way, so they share one heading treatment as well as one animation.

## The mascot is a dial, not a player

`MascotView` exposes `set_mood(mood, intensity, hold, immediate)` and nothing
else. Callers name a gameplay beat's meaning, not a frame or a clip, so the
mapping from beat to expression lives in one place
(`GameplayHudLayer.react_to_*`) and gameplay code never learns the art's
structure.

Three properties are load-bearing:

- **Position is a float, eased.** Jumping from idle to "happiest" travels
  through every intermediate pose. This is what turns eight frames into
  animation.
- **Adjacent frames cross-fade, base opaque.** The lower frame is drawn opaque
  and the upper over it at the fractional alpha. Drawing both semi-transparent
  is the intuitive approach and produces a visible dip in opacity, because the
  gold ring is identical in every frame and two half-opaque copies of it sum to
  ~75%.
- **Track changes rewind through neutral.** Happy and sad share only the first
  pose, so a mid-expression flip queues the new mood and returns to neutral
  first rather than cutting the face.

Frames are a static cache shared by every instance: the HUD, a popup and the
level screen can all be alive at once.

## Reminder rules live in GDScript, scheduling lives in Java

`NotificationService` decides **whether** a reminder is warranted, **when** it
should land, and **what** it says. The Android plugin only schedules, cancels,
and reports permission state.

The split is deliberate and is the reason the feature is testable: native code
is reachable only from a device, so anything that decided something there would
be permanently unverified. Every rule is a pure function over a settings
dictionary and today's mission state.

The gate that matters is `should_remind()`: no reminder is scheduled unless
something is actually outstanding. It is re-evaluated on every mission-state
change rather than once a day, so claiming the last mission cancels the pending
alarm. A notification that fires on a day the player already finished everything
is how a channel gets muted.

`is_available()` returning false - desktop, or any build without the plugin -
must remain a silent no-op. The game must never depend on the reminder existing.

## The gameplay HUD's top row has a tenant

`objective_stack_anchor` (SHOTS LEFT, target card) is a separately anchored
container added after the top utility row, so it draws over it, and its vertical
position tracks the table rather than the HUD. Anything placed in the utility
row must share the coins card's baseline and height, or it disappears behind the
objective stack on limited-shots levels - which is exactly what happened to the
mascot at full band height.

# Architecture Addendum - Level Select Map and Milestone Chests

## A ScrollContainer owns scrolling; LevelMapView is its content

**Superseded 2026-09-07.** The map previously owned its own scroll offset, drag
velocity, exponential friction and draw transform. It no longer owns any of
them.

`LevelSelectOverlayLayer` owns a `ScrollContainer`. `LevelMapView` is its
content: it declares `custom_minimum_size.y = content_height()` - roughly
186,000 pixels for the thousand-level look-ahead - draws in content space with
no transform of its own, and is told which slice is on screen through
`set_window(top, height)`, wired to the container's vertical scrollbar
`value_changed` and to its `resized` signal.

The 186,000-pixel child is affordable because a `ScrollContainer` **positions**
its child on scroll rather than re-measuring it. Layout happens on `configure()`
and on resize; drawing stays windowed via `_visible_slot_range()` and repaints
only when the visible slot range changes. The height therefore costs a layout,
not a repaint.

The load-bearing rule is that **the map must not consume a drag**. It is
`MOUSE_FILTER_PASS`, its `_gui_input` handles only touch and mouse-button
events, and it never calls `accept_event()` on a drag or on a release that
travelled further than `TAP_MOVE_TOLERANCE`. `MOUSE_FILTER_STOP` is what stopped
the container seeing a drag when this screen was first built; every subsequent
workaround, including the hand-rolled flick physics this section used to
describe, was compensating for that rather than fixing it. Selection remains
release-based so a flick starting on a level plate scrolls instead of opening a
level.

`LevelSelectOverlayLayer` still owns no progression rules; it reads controller
snapshots and drives `scroll.scroll_vertical` through
`LevelMapView.scroll_offset_for_level()`.
...
- **Only the visible window is drawn.** The parent `ScrollContainer` owns the
  ...
  that call the view would redraw a thousand levels to show eight. The repaint
  is additionally gated on the visible slot range actually changing, so the many
  scroll events a single flick produces cost nothing between rows.
...
UI tap.
# Brand, Mascot, Mobile Scroll, and Notification Delivery Addendum

- Brand sources remain immutable under `assets/logo/`; only matching semantic runtime copies are consumed. The gradient/logo splash is generated by `prepare_brand_refresh_v1.gd` and never participates in gameplay rendering.
- `MascotView` consumes two 12-frame tracks generated from the v2 supplied sheet. All frames share one 160x160 registration; mood state remains a float, with cross-fade and neutral rewind owned entirely by presentation.
- `LevelMapView` owns custom drawing only; `LevelSelectOverlayLayer` owns the `ScrollContainer` and all scrolling. **Superseded 2026-09-07:** the claim that a screen drag must adjust the parent scroll offset directly, because "Android gesture capture over a custom Control is not reliable enough to delegate implicitly", was wrong. Capture failed only because the map was `MOUSE_FILTER_STOP`. With `MOUSE_FILTER_PASS` and no `accept_event()` on drags, the container receives every gesture and its own touch scrolling works normally. See the Level Select Map addendum above.
- `NotificationService` owns all state/timing/copy decisions. The Android `MajesticNotifications` singleton owns only permission, alarm scheduling/cancellation, and delivery through `ReminderReceiver`.

...
- `LevelMapView` owns custom drawing and touch drag forwarding; `LevelSelectOverlayLayer` still owns the `ScrollContainer`. A screen drag adjusts the parent scroll offset directly because Android gesture capture over a custom Control is not reliable enough to delegate implicitly.

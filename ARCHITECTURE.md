...
`FirebaseAnalyticsPlugin` remains registered by the persistent custom-template v2 manifest metadata. It explicitly returns `logEvent` from `getPluginMethods()` as well as retaining `@UsedByGodot`; this is required because a Godot 4.6.3 release-device run registered the singleton but omitted the annotation-discovered method. It retries `FirebaseAnalytics.getInstance()` when an event arrives if the Godot Activity was unavailable during plugin construction, translates JSON primitives to `Bundle` values, forwards to Firebase, logs native success/failure, and returns a boolean. Firebase automatic collection does not use this bridge.

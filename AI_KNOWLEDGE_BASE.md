...
- Level-entry presentation must wait while `ScreenTransitionLayer` is busy, then restore exact `GameConfig` table center, render scale, calm modulation, and `GemSpriteLayer` origin/opacity. Reset it on restart before a new attempt is configured.
...
- **Horizontal margins come from silhouette, not colour.** The gloss is a smooth horizontal gradient and stretches cleanly; only the ornamental cap must stay fixed. Measure where the opaque outline stops changing height. A colour-uniformity metric wrongly flags the gradient and yields absurd caps.
- A control shorter than `top + bottom` texture margins overlaps its own caps and visibly crushes. `run_ui_kit_polish_v1_tests` walks every button on every screen and enforces this; it found six crushed buttons that eye review had missed across three passes.
- Button heights are a contract, not a style preference: `UiDesignSystem.BUTTON_HEIGHT` / `HERO_BUTTON_HEIGHT` / `BANNER_HEIGHT`. Never hardcode a kit button height.
- Content (padding) margins must be re-derived whenever plate margins change, or text collides with the caps.

# 2026-08-29 - Presentation guard and typography guardrails

- A "do not present twice" guard on a shared overlay must allow **mode changes**. `ResultOverlayLayer.present()` blocked the fail screen while the out-of-shots rescue was up, so GIVE UP silently did nothing and Skip looked unavailable on limited-shots levels. Guard on "same mode already shown", not on "anything is shown".
- Nunito Sans at weight 800 is for headings and numbers, not prose. Running copy at 800 reads as a heavy block; use ~620 for paragraphs (`UiDesignSystem.font()`) and `heavy_font()` only where weight is the point.
- The kit plates carry wide ornamental caps, so button padding and font size trade against each other. Raising shared padding has twice overflowed the daily-missions card row past 720px - `run_ui_kit_polish_v1_tests` now asserts modals fit the viewport. Compact contexts (cards) override with their own tighter padding rather than shrinking the shared value.
- HUD readouts that change during play belong in the centred objective stack at score size with a pulse on change. A small label parked beside Coins is not readable as a live counter.
- One-time teaching state lives in its own `tutorial` save section and is recorded **before** presenting, so a force-quit mid-briefing under-teaches once rather than re-teaching on every launch.

# 2026-08-29 - UI interaction guardrails

- **One silhouette per button family.** Godot swaps the stylebox on hover/press; if states point at different textures the plate morphs mid-interaction. Vary tint only, and carry press motion with scale. `run_ui_kit_polish_v1_tests.gd` enforces this.
- Never use `btn_square_swap` as an interaction state - its swap arrows are painted into the artwork, so it turns whatever icon sits on it into swap arrows.
- Secondary plates must start *recessed* (tint below 1.0). At full brightness they out-shout the primary action and the screen reads as four competing buttons.
- Any plate generated by a tool script **must be imported before use**. `load()` on an unimported PNG returns null and the button silently draws nothing - it does not error.
- Screen transitions must apply their state swap **synchronously**; only the reveal may be animated. Deferring the swap into a tween callback makes every navigation call async and silently breaks callers that read `app_flow_state` afterwards.
- Reward celebrations fire only after the reward is persisted and banked, never on request, so feedback can never imply a reward the player did not receive.
- A capture harness must verify the scene actually instantiated (`has_method("hud_snapshot")`). A failed script load leaves a bare `Node2D` and the harness will otherwise report PASS over blank screenshots.

# 2026-08-29 - UI kit, typography, and daily-mission state guardrails

- `NunitoSans-Variable.ttf` defaults its `wght` axis to **200 (ExtraLight)**. Always set the axis explicitly through `FontVariation.variation_opentype` (tag from `TextServerManager.get_primary_interface().name_to_tag("wght")`). Loading the variable font plain is why the face renders spindly.
- Nine-patch margins must stay **below half the shortest height** an asset is drawn at. Using the art's natural rim thickness (e.g. 48px vertical on a 68px-tall button) makes the unstretched caps overlap and visibly crushes the plate. Kit button verticals are 26-34.
- The kit's plates carry wide ornamental caps, so two captioned buttons cannot share a row at 720px design width without overflowing. Stack modal actions vertically.
- A colour tint cannot desaturate green. Disabled green actions swap to the neutral plate outright; a dimmed CLAIM still reads as available.
- Only assets in `UiKit.NINE` may be nine-patched. Fixed-composition art has no correct slicing — draw it with `UiKit.texture_rect()`.
- Keep `DailyMissionService` pure. It previously mutated the caller's Dictionary and returned that same object, which made `daily_state != previous` always false and silently disabled every save, and let a failed save consume a claim without paying it. Use the explicit `changed` flag, and assign the returned state only after the save succeeds.
- Overlay layers: HUD 40, Result 50, Home 60, Daily Missions 65. A popup opened from Home must sit above 60, or it renders behind an opaque screen and appears to do nothing.
- `present_out_of_shots()` relabels the Home button; `present()` must restore it. Any state a specialised presenter mutates has to be reset by the general one.
- Do not hardcode economy costs in tests. Derive them from `GameConfig`; a `SKIP_LEVEL_COST` retune previously left seven assertions red.
